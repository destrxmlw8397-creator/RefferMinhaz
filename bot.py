...
# --- KEYBOARDS ---
main_buttons = [
    [Button.text('ðŸ’° Balance', resize=True), Button.text('ðŸ‘« Invite', resize=True)],
    [Button.text('ðŸŒ¾ Staking', resize=True), Button.text('ðŸ“¤ Withdraw', resize=True)],
    [Button.text('ðŸ“Š Statistics', resize=True), Button.text('ðŸ“‹ Task', resize=True)]
]
...
admin_keyboard = [
    ...
    [Button.text('ðŸŽ Set Bonus'), Button.text('ðŸ“Š Statistics'), Button.text('Broadcast')],
...
ADMIN_COMMANDS = [
    ...
    "Add Balance", "Cut Balance", "ðŸŽ Set Bonus", "ðŸ“Š Statistics", "Broadcast",
...
USER_COMMANDS = [
    "ðŸ’° Balance", "ðŸ‘« Invite", "ðŸŒ¾ Staking",
    "ðŸ“¤ Withdraw", "ðŸ“Š Statistics", "ðŸ“‹ Task"
]
...
async def get_invite_data(user_id, bot_username):
    ...
    msg = (f"ðŸ™Œ **Total Refers = {total_ref} User(s)**\n\n"
           f"ðŸ™Œ **Your Invite Link =** {link}\n\n"
           f"ðŸ§¶ **Invite to Earn {ref_bonus} {currency} Per Invite**")
    kb = [[Button.inline("ðŸ” My Refers", b"my_ref"), Button.inline("ðŸ”¥ Top List", b"top_list")]]
    return msg, kb
...
def build_channel_buttons(channels):
    ...
    if row:
        rows.append(row)
    rows.append([Button.inline("ðŸŸ¢ Joined", b"check_join")])
    return rows
...
async def grant_milestone_bonuses(user_id, sets):
    ...
    if len(new_milestones) == 1:
        msg = f"ðŸŽ‰ **Congratulations!** You have reached **{new_milestones[0]}** referrals and earned a bonus of **{bonus_amt} {sets['currency']}**!"
    else:
        milestones_str = ', '.join(map(str, new_milestones))
        msg = f"ðŸŽ‰ **Congratulations!** You have reached milestones {milestones_str} and earned a total of **{total_bonus} {sets['currency']}**!"
    try:
            ...
            if session.get('task_type') == 'Media':
                ...
                if proof_type == 'skip':
                    ...
                    async with db_pool.acquire() as conn:
                        ...
                        if existing:
                            await client.edit_message(chat_id, message_id, "âœ… You have already claimed this task.")
                            return
                        ...
                        if task['completed_count'] >= task['task_limit']:
                            await client.edit_message(chat_id, message_id, "âŒ This task is already full.")
                            return
                    ...
                    buttons = [[Button.inline("ðŸ”™ Back", b"task_back")]]
                    ...
                    buttons = [
                        [Button.inline("ðŸŽ Claim Reward", f"claim_task_{task_id}")],
                        [Button.inline("ðŸ”™ Back", b"task_back")]
                    ]
                    await client.edit_message(chat_id, message_id, text, buttons=buttons)
...
async def show_task_list(user_id, chat_id, msg_id=None, task_type=None):
    ...
    if available:
        ...
        for idx, (tid, ttype, url, ttime, reward, tlimit, ccount, ptype) in enumerate(available, start=1):
            label = f"ðŸ“Œ {ttype} Task {idx}"
            rows.append([Button.inline(label, f"start_task_{tid}")])
        rows.append([Button.inline("âŒ Cancel", b"task_cancel_list")])
        text = f"ðŸ“‹ **Available {task_type if task_type else ''} Tasks**\n\nClick a task to start:"
        if msg_id:
        ...
        if msg_id:
            await client.edit_message(chat_id, msg_id, f"âŒ No active {task_type if task_type else ''} tasks available.")
        else:
            await client.send_message(chat_id, f"âŒ No active {task_type if task_type else ''} tasks available.")
...
async def render_task_details(user_id, chat_id, msg_id):
    ...
    settings = await get_settings()
    currency = settings['currency']
    progress_text = f"ðŸ“Š **Task Progress:** {ccount}/{tlimit}"
    if ptype == 'skip':
        proof_display = "ðŸ“Ž No proof required"
    else:
        proof_display = "ðŸ“¸ Screenshot" if ptype == "screenshot" else "ðŸŽ¥ Screen Record"
    if task_type == 'Media':
        text = (f"ðŸ“Œ **Task: Media Task {task_id}**\n\n"
                f"â±ï¸ **Time Required:** {ttime} seconds\n"
                f"ðŸ’° **Reward:** {reward} {currency}\n"
                f"{progress_text}\n"
                f"{proof_display}\n\n")
        if ptype == 'skip':
            text += (f"1ï¸âƒ£ Click the **Visit Website** button below.\n"
                     f"2ï¸âƒ£ After visiting, click **Check** to start the timer.\n"
                     f"3ï¸âƒ£ Wait {ttime} seconds for the timer to finish.\n"
                     f"4ï¸âƒ£ After timer, reward will be automatically added to your balance.\n\n"
                     f"âš ï¸ **Note:** You must actually visit the website to get the reward.")
        else:
            text += (f"1ï¸âƒ£ Click the **Visit Website** button below.\n"
                     f"2ï¸âƒ£ After visiting, click **Check** to start the timer.\n"
                     f"3ï¸âƒ£ Wait {ttime} seconds for the timer to finish.\n"
                     f"4ï¸âƒ£ After timer, click **Claim Reward** and submit your proof.\n\n"
                     f"âš ï¸ **Note:** You must actually visit the website and submit a valid proof to get the reward.")
        buttons = [
            [Button.url("ðŸ”— Visit Website", fixed_url)],
            [Button.inline("âœ… Check", f"task_visited_{task_id}")],
            [Button.inline("ðŸ”™ Back", b"task_back")]
        ]
    else:  # TG Task
        text = (f"ðŸ“Œ **Task: TG Task {task_id}**\n\n"
                f"ðŸ’° **Reward:** {reward} {currency}\n"
                f"{progress_text}\n\n"
                f"1ï¸âƒ£ Click the **Join Channel** button below.\n"
                f"2ï¸âƒ£ After joining, click **Check** to verify membership.\n"
                f"3ï¸âƒ£ If you are a member, you can **Claim Reward**.\n\n"
                f"âš ï¸ **Note:** You must join the channel to get the reward.")
        buttons = [
            [Button.url("ðŸ”— Join Channel", fixed_url)],
            [Button.inline("âœ… Check", f"tg_check_{task_id}")],
            [Button.inline("ðŸ”™ Back", b"task_back")]
        ]
...
async def send_task_notification(task_id, task_type, url, reward, task_limit, proof_type=None, time_required=None):
    ...
    try:
        ...
        if task_type == "Media":
            proof_text = f"ðŸ“Ž Proof Type: {proof_type}\n" if proof_type else ""
            time_text = f"â±ï¸ Time Required: {time_required} seconds\n" if time_required else ""
            detail_text = (
                f"ðŸ“Œ **New Media Task Added!**\n\n"
                f"ðŸ”— URL: {url}\n"
                f"{time_text}"
                f"ðŸ’° Reward: {reward} {currency}\n"
                f"ðŸ‘¥ User Limit: {task_limit}\n"
                f"{proof_text}"
            )
        else:
            detail_text = (
                f"ðŸ“Œ **New TG Task Added!**\n\n"
                f"ðŸ“¢ Channel: https://t.me/{url}\n"
                f"ðŸ’° Reward: {reward} {currency}\n"
                f"ðŸ‘¥ User Limit: {task_limit}\n"
            )
        start_url = f"https://t.me/{bot_username}?start=task_{task_id}"
        buttons = [
            [Button.url("ðŸš€ Start Task", start_url)]
        ]
...
async def start_task_for_user(user_id, task_id):
    async with db_pool.acquire() as conn:
        ...
        if existing:
            ...
            await client.send_message(user_id, f"âŒ **Already {status_text.title()}**\n\nYou have already {status_text} this task. You cannot start it again.")
            return False
    ...
    chat_id = user_id
    msg = await client.send_message(user_id, "ðŸ“Œ **Task Details**\n\nLoading...")
    msg_id = msg.id
...
async def start(event):
    ...
    if msg_text.startswith('/start task_'):
        try:
            ...
            await event.respond("âŒ Invalid task link.")
            return
        async with db_pool.acquire() as conn:
        ...
        if not existing and msg_text.startswith('/start '):
            ...
            if len(parts) > 1:
                ...
                if potential_ref != user_id:
                    ...
                    ref_alert = f"ðŸ”‹ You Got a New Referral {ref_name_link}\n\nðŸ’¡ Reward Only If Referral Solves the Captcha and Joins Our Channels"
                    try:
    ...
    if is_new_user and welcome_bonus > 0:
        welcome_msg = (f"ðŸ‘‹ Welcome {name}!\n\n"
                       f"ðŸŽ‰ **Congratulations!** You have received a Welcome Bonus of **{welcome_bonus:.2f} {currency}** in your Hold Balance. It will be released weekly.")
    else:
        welcome_msg = f"ðŸ‘‹ Welcome All Forwarder!"

    if not await is_admin(user_id) and not await is_user_joined_all(user_id):
        async with db_pool.acquire() as conn:
            channels = await conn.fetch("SELECT channel_username FROM required_channels")
        rows = build_channel_buttons([(c['channel_username'],) for c in channels])
        join_msg = "â›” Must Join All Required Channels\n\nClick each button below to join, then press 'Joined'."
        await event.respond(join_msg, buttons=rows)
    else:
...
async def check_join_callback(event):
    ...
    if await is_user_joined_all(user_id):
        ...
        welcome_bonus = settings['welcome_bonus']
        currency = settings['currency']
        welcome_msg = f"ðŸ‘‹ Welcome {event.sender.first_name}!"
        if welcome_bonus > 0:
            welcome_msg += f"\n\nðŸŽ‰ **Congratulations!** You have received a Welcome Bonus of **{welcome_bonus:.2f} {currency}** in your Hold Balance. It will be released weekly."
        await event.respond(welcome_msg, buttons=main_buttons)
    else:
        async with db_pool.acquire() as conn:
            channels = await conn.fetch("SELECT channel_username FROM required_channels")
        rows = build_channel_buttons([(c['channel_username'],) for c in channels])
        await event.edit("â›” You haven't joined all channels yet.\nClick each button below to join, then press 'Joined'.", buttons=rows)
...
async def admin_panel(event):
    ...
    await event.respond("ðŸ›  **Admin Panel**", buttons=admin_keyboard)
...
async def handle_text(event):
        ...
        if text in ADMIN_COMMANDS or text in CHANNEL_SETTINGS_COMMANDS:
            ...
            if step == 'add':
                ...
                if not channel_username:
                    await event.respond("âŒ Invalid channel. Please provide a valid channel username or link.")
                    return
                async with db_pool.acquire() as conn:
                    if category == 'joining':
                        try:
                            await conn.execute("INSERT INTO required_channels (channel_username) VALUES ($1)", channel_username)
                            await event.respond(f"âœ… Channel @{channel_username} added to joining list.")
                        except asyncpg.UniqueViolationError:
                            await event.respond(f"âš ï¸ Channel @{channel_username} already exists in joining list.")
                    elif category == 'withdrawal':
                        await conn.execute("UPDATE settings SET withdrawal_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Withdrawal channel set to @{channel_username}.")
                    elif category == 'proof':
                        await conn.execute("UPDATE settings SET task_proof_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Proof channel set to @{channel_username}.")
                    elif category == 'task':
                        await conn.execute("UPDATE settings SET task_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Task channel set to @{channel_username}.")
                    else:
                        await event.respond("âŒ Unknown category.")
                del admin_channel_state[user_id]
                ...
                async with db_pool.acquire() as conn:
                    if category == 'joining':
                        try:
                            ...
                            await event.respond(f"âœ… Joining channel updated to @{channel_username}.")
                        except asyncpg.UniqueViolationError:
                            await event.respond(f"âš ï¸ Channel @{channel_username} already exists.")
                    elif category == 'withdrawal':
                        await conn.execute("UPDATE settings SET withdrawal_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Withdrawal channel updated to @{channel_username}.")
                    elif category == 'proof':
                        await conn.execute("UPDATE settings SET task_proof_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Proof channel updated to @{channel_username}.")
                    elif category == 'task':
                        await conn.execute("UPDATE settings SET task_channel = $1 WHERE id=1", channel_username)
                        await event.respond(f"âœ… Task channel updated to @{channel_username}.")
                    else:
                        await event.respond("âŒ Unknown category.")
                del admin_channel_state[user_id]
                await event.respond("Channel Settings", buttons=channel_settings_keyboard)
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and user_id in admin_edit_state and admin_edit_state[user_id].get('stage') == 'awaiting_input':
        ...
        if not field or not col:
            await event.reply("âŒ Invalid edit session.")
            return
        valid = True
        error_msg = None
        new_val = None
        ...
        if not valid:
            await event.reply(f"âŒ {error_msg}\nPlease try again.")
            return
        ...
        confirm_text = (f"âš ï¸ **Confirm {display} Update**\n\n"
                        f"Current: {current_str}\n"
                        f"New: {new_str}\n\n"
                        f"Are you sure?")
        buttons = [
            [Button.inline("âœ… Yes", f"edit_confirm_{tid}_{field}_yes"),
             Button.inline("ðŸ”™ Back", f"edit_back_{tid}")]
        ]
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and user_id in admin_tg_task_state:
        ...
        if text in ADMIN_COMMANDS or text in TASK_SETTINGS_COMMANDS:
            ...
            if step == 'channel':
                ...
                bot_username = (await client.get_me()).username
                msg = (f"ðŸ“¢ Target Channel: https://t.me/{channel_username}\n\n"
                       ...
                       f"3. Click the \"Check âœ…\" button below once done.")
                buttons = [
                    [Button.inline("âœ… Check", f"tg_admin_check_{channel_username}")],
                    [Button.inline("âœï¸ Edit", f"tg_admin_edit_{channel_username}")],
                    [Button.inline("âŒ Cancel", b"tg_admin_cancel")]
                ]
                ...
                try:
                    limit = int(text)
                    if limit < 5:
                        await event.respond("âŒ Minimum limit is 5. Please enter a number >= 5.")
                        return
                    state['limit'] = limit
                    state['step'] = 'reward'
                    currency = settings['currency']
                    msg = (f"ðŸ’° Step 2: Set Task Reward\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"
                           f"ðŸŽ¯ Target Channel: https://t.me/{state['channel']}\n"
                           f"ðŸ“Š Task Limit: {limit}\n\n"
                           f"âš ï¸ Minimum per Task price: 0.005 {currency}\n"
                           f"Please send your per task reward ðŸ‘‡ðŸ»")
                    await event.respond(msg)
                except ValueError:
                    await event.respond("âŒ Invalid number. Please enter a valid integer.")
            elif step == 'reward':
                try:
                    reward = float(text)
                    if reward <= 0:
                        await event.respond("âŒ Reward must be positive.")
                        return
                    state['reward'] = reward
                    state['step'] = 'confirm'
                    currency = settings['currency']
                    msg = (f"âš ï¸ **Confirm TG Task Creation**\n\n"
                           f"ðŸ“¢ Target Channel: https://t.me/{state['channel']}\n"
                           f"ðŸ‘¥ Task Limit: {state['limit']}\n"
                           f"ðŸ’° Reward per task: {reward} {currency}\n\n"
                           f"Are you sure?")
                    buttons = [
                        [Button.inline("âœ… Yes", f"tg_admin_confirm_yes")],
                        [Button.inline("âŒ No", f"tg_admin_confirm_no")]
                    ]
                    await event.respond(msg, buttons=buttons)
                except ValueError:
                    await event.respond("âŒ Invalid reward amount. Please enter a number.")
            return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and user_id in task_waiting:
        ...
        if text in ADMIN_COMMANDS or text in TASK_SETTINGS_COMMANDS:
            ...
            if step == 'url':
                ...
                action['step'] = 'time'
                await event.respond("â±ï¸ Enter time required in seconds (e.g., 30):")
                return
            elif step == 'time':
                try:
                    time_val = int(text)
                    if time_val <= 0:
                        await event.respond("âŒ Time must be a positive number. Enter again:")
                        return
                    action['task_data']['time_required'] = time_val
                    action['step'] = 'reward'
                    currency = settings['currency']
                    await event.respond(f"ðŸ’° Enter reward amount (in {currency}) for completing this task:")
                    return
                except ValueError:
                    await event.respond("âŒ Invalid number. Enter time in seconds (e.g., 30):")
                    return
            elif step == 'reward':
                try:
                    reward = float(text)
                    if reward <= 0:
                        await event.respond("âŒ Reward must be positive. Enter again:")
                        return
                    action['task_data']['reward'] = reward
                    action['step'] = 'limit'
                    await event.respond("ðŸ‘¥ How many users can complete this task? (Enter a number):")
                    return
                except ValueError:
                    await event.respond("âŒ Invalid reward amount. Enter a number:")
                    return
            elif step == 'limit':
                try:
                    limit = int(text)
                    if limit <= 0:
                        await event.respond("âŒ Limit must be positive. Enter again:")
                        return
                    action['task_data']['task_limit'] = limit
                    action['step'] = 'proof_type'
                    await event.respond("ðŸ“Ž Choose proof type:\nSend `screenshot`, `screen record`, or `skip`")
                    return
                except ValueError:
                    await event.respond("âŒ Invalid limit. Enter a number:")
                    return
            elif step == 'proof_type':
                ptype = text.strip().lower()
                if ptype not in ['screenshot', 'screen record', 'skip']:
                    await event.respond("âŒ Invalid choice. Please send `screenshot`, `screen record`, or `skip`")
                    return
                action['task_data']['proof_type'] = ptype
                task_data = action['task_data']
                currency = settings['currency']
                confirm_text = (f"âš ï¸ **Confirm Add Media Task**\n\n"
                                f"ðŸ”— URL: {task_data['url']}\n"
                                f"â±ï¸ Time: {task_data['time_required']} seconds\n"
                                f"ðŸ’° Reward: {task_data['reward']} {currency}\n"
                                f"ðŸ‘¥ User Limit: {task_data['task_limit']}\n"
                                f"ðŸ“Ž Proof Type: {task_data['proof_type']}\n\n"
                                f"Are you sure?")
                kb = [
                    [Button.inline("âœ… Yes", b"confirm_task_yes")],
                    [Button.inline("âŒ No", b"confirm_task_no")]
                ]
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(participant.participant, (ChannelParticipantAdmin, ChannelParticipantCreator)):
                        ...
                        await event.respond(f"âœ… Task proof channel set to: {channel_str}")
                    else:
                        await event.respond("âŒ Bot is not an admin of this channel. Please make the bot admin and try again.")
                except UserNotParticipantError:
                    await event.respond("âŒ Bot is not a member of this channel. Please add the bot and make it admin.")
                except Exception as e:
                    await event.respond(f"âŒ Error checking admin status: {e}")
            except Exception as e:
                await event.respond(f"âŒ Could not resolve channel: {e}\nPlease send a valid channel username (with @) or channel ID.")
            del admin_waiting[user_id]
        return
                        ...
                        if action == "wb":
                            ...
                            for u in all_users:
                                try:
                                    ...
                                    pass
                            msg = f"âœ… Broadcast finished. Sent to {count} users."
                    del admin_waiting[user_id]
                    await event.respond(msg)
                    return
        else:
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True):
        if text == "Set Welcome Bonus":
            ...
            msg = (f"ðŸ“Š **Bot Statistics**\n\n"
                   ...
                   f"ðŸ’° Total Payouts: {total_payout:.2f} {currency}\n"
                   f"â³ Pending Withdrawals: {pending_requests} requests\n"
                   f"ðŸ’µ Pending Amount: {pending_amount:.2f} {currency}\n"
                   f"ðŸ’Ž Total Real Balance: {total_balance:.2f} {currency}\n"
                   f"ðŸ”„ Total Hold Balance: {total_hold:.2f} {currency}")
            await event.respond(msg)
            return
        elif text == "Broadcast":
            ...
            await event.respond("âš™ï¸ **Task Settings**\n\nSelect an option:", buttons=task_settings_keyboard)
            return
        elif text == "Channel Settings":
            await event.respond("ðŸ”§ **Channel Settings**\n\nSelect a category to manage channels:", buttons=channel_settings_keyboard)
            return
        elif text == "Add Admin":
            if user_id != MAIN_ADMIN_ID:
                await event.respond("âŒ Only the main admin can manage admins.")
                return
            admin_waiting[user_id] = "add_admin"
            await event.respond("ðŸ“¢ Send the user ID or username (with @) of the new admin:")
            return
            ...
            if not admins:
                await event.respond("â„¹ï¸ No other admins to remove.")
                return
            rows = []
            for (admin_id,) in admins:
                try:
                    ...
                    name = str(admin_id)
                rows.append([Button.inline(f"âŒ {name}", f"del_admin_{admin_id}")])
            rows.append([Button.inline("ðŸ”™ Cancel", b"cancel_del_admin")])
        ...
        if text in ADMIN_COMMANDS:
            ...
            try:
                ...
                if new_admin_id == MAIN_ADMIN_ID:
                    await event.respond("âŒ The main admin is already an admin.")
                    del admin_waiting[user_id]
                    return
                async with db_pool.acquire() as conn:
                    existing = await conn.fetchval("SELECT 1 FROM admins WHERE user_id=$1", new_admin_id)
                    if existing:
                        await event.respond("âš ï¸ This user is already an admin.")
                    else:
                        await conn.execute("INSERT INTO admins (user_id) VALUES ($1)", new_admin_id)
                        await event.respond(f"âœ… User {entity.first_name} (ID: {new_admin_id}) has been added as an admin.")
            except Exception as e:
                await event.respond(f"âŒ Could not find user: {e}")
            del admin_waiting[user_id]
            return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and text in CHANNEL_SETTINGS_COMMANDS:
        if text == "Back":
            await event.respond("ðŸ”™ Back to Admin Panel.", buttons=admin_keyboard)
            return
            ...
            await event.respond(f"ðŸ“¢ Please send the channel username or link for **{text}**:")
            return
            ...
            buttons = [
                ...
                [Button.inline("âŒ Cancel", b"edit_cat_cancel")]
            ]
            await event.respond("ðŸ“ Select a category to edit a channel:", buttons=buttons)
            return
            ...
            buttons = [
                ...
                [Button.inline("âŒ Cancel", b"del_cat_cancel")]
            ]
            await event.respond("ðŸ—‘ï¸ Select a category to delete a channel:", buttons=buttons)
            return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and text in TASK_SETTINGS_COMMANDS:
        if text == "Back":
            ...
            await event.respond("ðŸ”— Enter the website URL (must include http:// or https://):")
            return
        elif text == "Add TG Task":
            admin_tg_task_state[user_id] = {'step': 'channel'}
            await event.respond("ðŸ“¢ Please send the channel username (e.g., @checkingreffer) or channel link (e.g., https://t.me/checkingreffer):")
            return
            ...
            if not tasks:
                await event.respond(f"âŒ No active {task_type} tasks found.")
                return
            rows = []
            for task in tasks:
                rows.append([Button.inline(f"âœï¸ {task['url'][:30]}...", f"edit_task_{task['id']}_{task_type}")])
            rows.append([Button.inline("ðŸ”™ Cancel", b"cancel_edit_task")])
            await event.respond(f"ðŸ“‹ Select a {task_type} task to edit:", buttons=rows)
            return
            ...
            rows = []
            for task in tasks:
                rows.append([Button.inline(f"ðŸ—‘ï¸ {task['url'][:30]}...", f"del_task_{task['id']}_{task_type}")])
            rows.append([Button.inline("ðŸ”™ Cancel", b"cancel_del_task")])
            await event.respond(f"ðŸ“‹ Select a {task_type} task to delete:", buttons=rows)
            return
        elif text == "Set Proof Channel":
            admin_waiting[user_id] = "set_proof_channel"
            await event.respond("ðŸ“¢ Send the channel username (with @) or channel ID (for private channels).\n\nMake sure the bot is an admin of that channel.")
            return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and user_id in admin_waiting and admin_waiting[user_id] in ["add_balance", "cut_balance", "set_bonus"]:
        ...
        if len(parts) != 2:
            await event.respond("âŒ Invalid format. Please send two values separated by space.")
            return
        try:
            ...
            if action in ["add_balance", "cut_balance"]:
                ...
                amount = second_val
                if amount <= 0:
                    await event.respond("âŒ Amount must be positive.")
                    return
                ...
                if ref_count <= 0 or bonus_amount <= 0:
                    await event.respond("âŒ Both values must be positive.")
                    return
                admin_confirm[user_id] = {'action': action, 'ref_count': ref_count, 'bonus_amount': bonus_amount}
            del admin_waiting[user_id]
            currency = settings['currency']
            if action == "set_bonus":
                confirm_text = (f"âš ï¸ **Confirm Set Bonus**\n\n"
                                f"Referral count: `{ref_count}`\n"
                                f"Bonus amount: {bonus_amount:.2f} {currency}\n\n"
                                f"Are you sure?")
            else:
                confirm_text = (f"âš ï¸ **Confirm {action.replace('_', ' ').title()}**\n\n"
                                f"User ID: `{target_user}`\n"
                                f"Amount: {amount:.2f} {currency}\n\n"
                                f"Are you sure?")
            kb = [
                [Button.inline("âœ… Yes", f"confirm_{action}_yes")],
                [Button.inline("âŒ No", f"confirm_{action}_no")]
            ]
            await event.respond(confirm_text, buttons=kb)
        except ValueError:
            await event.respond("âŒ Invalid numbers. Please enter numeric values.")
        return
    ...
    if user_id in screenshot_waiting:
        ...
        if proof_type == 'screenshot' and event.message.photo:
            ...
            await event.reply("ðŸ“¸ Please send a **photo** (screenshot) of the website. Text messages are not accepted.")
            return
        elif proof_type == 'screen record' and not event.message.video:
            await event.reply("ðŸŽ¥ Please send a **video** (screen recording) of the website. Text messages are not accepted.")
            return
        else:
    ...
    if user_id in waiting_users:
        ...
        if state == 'set_wallet':
            ...
            kb = [[Button.inline("âœ… Confirm Wallet", b"confirm_wallet")],
                  [Button.inline("âŒ Change", b"change_wallet")]]
            await event.reply(f"ðŸ—‚ Your wallet has been set to:\n`{text}`\n\nPlease confirm it is correct.", buttons=kb)
        ...
        # State: change_wallet (user wants to change wallet, send new address)
            ...
            kb = [[Button.inline("âœ… Confirm", b"confirm_change_wallet")],
                  [Button.inline("âŒ Cancel", b"cancel_change_wallet")]]
            await event.reply(f"ðŸ“ New wallet address:\n`{text}`\n\nPlease confirm.", buttons=kb)
        ...
        # State: withdraw_amount (user sends amount)
            ...
            try:
                amount = float(text)
                if amount <= 0:
                    await event.reply("âŒ Amount must be positive. Please enter a valid number.")
                    return
                # Check balance and min
                async with db_pool.acquire() as conn:
                    user = await conn.fetchrow("SELECT balance, wallet FROM users WHERE user_id=$1", user_id)
                    if not user:
                        await event.reply("âŒ User not found.")
                        del waiting_users[user_id]
                        return
                    balance = user['balance']
                    wallet = user['wallet']
                    if amount > balance:
                        await event.reply(f"âŒ You only have {balance:.2f} {settings['currency']} in real balance. Please enter a lower amount.")
                        return
                    if amount < settings['min_withdraw']:
                        await event.reply(f"âŒ Minimum withdrawal is {settings['min_withdraw']} {settings['currency']}.")
                        return
                # Calculate fee
                ...
                net = amount - fee
                currency = settings['currency']
                confirm_text = (f"ðŸ“¤ **Withdrawal Confirmation**\n\n"
                                f"ðŸ’° Total Requested: {amount:.2f} {currency}\n"
                                f"ðŸ“‰ Transaction Fee ({fee_percent}%): -{fee:.2f} {currency}\n"
                                f"âœ… Net Amount Credited: {net:.2f} {currency}\n"
                                f"ðŸ’³ Wallet Address: `{wallet}`\n\n"
                                f"âš ï¸ Please confirm that the wallet address is correct.\n"
                                f"Click **Confirm** to proceed with withdrawal.")
                kb = [
                    [Button.inline("âœ… Confirm & Withdraw", f"confirm_withdraw_{user_id}")],
                    [Button.inline("âŒ Cancel", b"cancel_withdraw")]
                ]
                ...
                await event.reply("âŒ Invalid amount. Please enter a number (e.g., 10.5).")
        else:
            # Unknown state, clear
            del waiting_users[user_id]
        return
    ...
    if not await is_admin(user_id) and not await is_user_joined_all(user_id):
        await event.respond("â›” You must join all required channels first. Use /start to check.")
        return
    ...
    if text == 'ðŸ’° Balance':
        ...
        msg = (f"ðŸ‘‘ Name - {user['name']} \nðŸ†” UserID - `{user['user_id']}`\n"
               f"ðŸ’° **Real Balance (Withdrawable)**: {real_bal:.2f} {currency}\n"
               f"ðŸ”„ **Hold Balance (Staking)**: {hold_bal:.2f} {currency}\n"
               f"ðŸ’Ž **Total**: {total:.2f} {currency}\n\n"
               f"ðŸŽ‰ Refer and earn more!")
        ...
        if join_timestamp:
            ...
            join_date = "Unknown"
        currency = settings['currency']
        msg = (f"ðŸ“Š **Your Statistics**\n\n"
               f"ðŸ‘¥ Total Referrals: {total_ref}\n"
               f"ðŸ’° Total Earned (Referrals+Tasks): {total_earned:.2f} {currency}\n"
               f"ðŸ’¸ Total Paid Out: {total_withdrawn:.2f} {currency}\n"
               f"ðŸ”„ Total Released from Hold: {total_released:.2f} {currency}\n"
               f"â³ Pending Payment: {pending_sum:.2f} {currency}\n"
        ...
        msg = (f"ðŸŒ¾ **Staking Dashboard**\n\n"
               f"ðŸ”„ **Current Hold Balance:** {hold_bal:.2f} {currency}\n"
               f"ðŸ’° **Daily Farming Reward:** {daily_bonus} {currency}\n"
               f"â³ **Next Claim Time:** {remaining}\n"
               f"ðŸ“… **Weekly Release:** {release_status}\n"
               f"ðŸ“ˆ **Total Released to Real Balance:** {total_released:.2f} {currency}\n\n"
               f"Claim your daily farming reward below.")
        buttons = [[Button.inline("ðŸŽ Claim Daily Reward", b"claim_farming")],
                   [Button.inline("ðŸ”™ Back", b"back_main")]]
        ...
        if pending:
            await event.reply("âš ï¸ You already have a pending withdrawal request. Please wait for it to be processed.")
            return
        if settings['withdraw_status'] == 0:
            await event.reply("âš ï¸ Withdrawal is currently OFF by Admin.")
            return
        real_bal = user['balance']
        if real_bal < settings['min_withdraw']:
            await event.reply(f"âš ï¸ Your real balance ({real_bal:.2f} {settings['currency']}) is below the minimum withdrawal amount of {settings['min_withdraw']} {settings['currency']}.")
            return
        ...
        if wallet == 'Not Set':
            ...
            await event.reply("ðŸ—‚ You have not set your wallet address. Please send your Paytm/UPI/Bank number now.\n\nThis will be used for all future withdrawals.")
            return
        else:
            # Show current wallet with options
            kb = [
                [Button.inline("âœ… Confirm", b"withdraw_confirm")],
                [Button.inline("âœï¸ Change", b"withdraw_change")],
                [Button.inline("âŒ Cancel", b"withdraw_cancel")]
            ]
            await event.reply(f"ðŸ’³ Your current wallet address:\n`{wallet}`\n\nDo you want to proceed with this wallet?", buttons=kb)
            # Set state to show wallet (handled by callback)
            waiting_users[user_id] = 'show_wallet'

    # --- TASK SYSTEM ---
    elif text == 'ðŸ“‹ Task':
        await event.respond("ðŸ“‹ **Task Menu**\n\nSelect an option below:", buttons=task_buttons)

    elif text in TASK_COMMANDS:
        if text == 'Back':
            await event.respond("ðŸ”™ Back to main menu.", buttons=main_buttons)
...
async def process_proof(event, user_id, media_type):
    async with screenshot_lock:
        ...
        async with db_pool.acquire() as conn:
            ...
            if existing:
                await event.reply("âš ï¸ You already have a pending submission for this task. Please wait for admin review.")
                return
        ...
        if not proof_channel_str:
            ...
            await event.reply("âŒ Task proof channel not set. Please contact admin. You can try again later.")
            if user_id in task_sessions:
                del task_sessions[user_id]
            return
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE tasks SET completed_count = completed_count - 1 WHERE id=$1", task_id)
                await conn.execute("DELETE FROM task_submissions WHERE id=$1", sub_id)
            await event.reply(f"âŒ Could not resolve proof channel. Error: {e}\nPlease contact admin.")
            if user_id in task_sessions:
                del task_sessions[user_id]
            return

        proof_type_display = "ðŸ“¸ Screenshot" if media_type == 'photo' else "ðŸŽ¥ Screen Record"

        caption = (f"ðŸ“¸ **New Task Submission** ({proof_type_display})\n\n"
                   f"ðŸ‘¤ **User:** {name}\n"
                   f"ðŸ†” **User ID:** `{user_id}`\n"
                   f"ðŸ“Œ **Task ID:** {task_id}\n"
                   f"ðŸ”— **URL:** {url}\n"
                   f"ðŸ’° **Reward:** {reward} {currency}\n"
                   f"ðŸ“… **Submitted:** {await get_now()}\n\n"
                   f"â³ **Status:** Pending Review")

        approve_data = f"approve_sub_{sub_id}"
        reject_data = f"reject_sub_{sub_id}"
        buttons = [
            [Button.inline("âœ… Approve", approve_data.encode()),
             Button.inline("âŒ Reject", reject_data.encode())]
        ]
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE tasks SET completed_count = completed_count - 1 WHERE id=$1", task_id)
                await conn.execute("DELETE FROM task_submissions WHERE id=$1", sub_id)
            await event.reply(f"âŒ Failed to submit proof. Please try again later. Error: {e}")
            if user_id in task_sessions:
                del task_sessions[user_id]
...
async def callback(event):
    ...
    if data == "change_wallet":
        if user_id in waiting_users and waiting_users[user_id] == 'confirm_wallet':
            ...
            await event.edit("âœï¸ Please send the new wallet address:")
    ...
    if data == "confirm_change_wallet":
        if user_id in waiting_users and waiting_users[user_id] == 'confirm_change_wallet':
            ...
            del waiting_users[user_id]
            await event.edit("âœ… Wallet updated successfully.")
            # Now ask for amount
            settings = await get_settings()
            async with db_pool.acquire() as conn:
                user = await conn.fetchrow("SELECT balance FROM users WHERE user_id=$1", user_id)
    ...
    if data == "cancel_change_wallet":
        if user_id in waiting_users:
            ...
            await event.edit("âŒ Wallet change cancelled.")
    ...
    if data == "withdraw_confirm":
        if user_id in waiting_users and waiting_users[user_id] == 'show_wallet':
            del waiting_users[user_id]
            await event.edit("âœ… Wallet confirmed. Now enter the amount you wish to withdraw.")
            settings = await get_settings()
            async with db_pool.acquire() as conn:
                user = await conn.fetchrow("SELECT balance FROM users WHERE user_id=$1", user_id)
                if user:
    ...
    if data == "withdraw_cancel":
        if user_id in waiting_users:
            del waiting_users[user_id]
            await event.edit("âŒ Withdrawal cancelled.")
            await event.answer("Cancelled.", alert=True)
        else:
            await event.answer("No pending withdrawal.", alert=True)
        return
    ...
    if data.startswith("confirm_withdraw_"):
        if user_id not in admin_confirm or admin_confirm[user_id].get('action') != 'withdraw':
            await event.answer("âŒ No pending withdrawal.", alert=True)
            return
        ...
        if target_user != user_id:
            await event.answer("âŒ Invalid user.", alert=True)
            return
        ...
        async with db_pool.acquire() as conn:
            ...
            if current_balance < amount:
                await event.edit(f"âŒ Insufficient balance. You have {current_balance:.2f} {settings['currency']}.")
                del admin_confirm[user_id]
                return
        ...
        del admin_confirm[user_id]
        await event.edit(f"âœ… **Withdrawal request submitted!**\n\n"
        ...
        if not withdrawal_channel:
            await event.respond("âŒ Withdrawal channel not set. Please contact admin.")
            return
        try:
            ...
            msg = (f"ðŸš€ NEW SUCCESSFUL WITHDRAWAL ðŸš€\n\n"
                   f"ðŸ‘¤ User: ID: {user_id}\n"
                   f"ðŸ’° Total Requested: {amount:.2f} {currency}\n"
                   f"ðŸ“‰ Transaction Fee ({settings['withdraw_fee']}%): -{fee:.2f} {currency}\n"
                   f"âœ… Net Amount Credited: {net:.2f} {currency}\n\n"
                   f"ðŸ’³ Wallet Address:\n`{wallet}`")
            pay_button = [Button.inline("âœ… Paid", f"p_{req_id}")]
            await client.send_message(withdrawal_entity, msg, buttons=pay_button)
        except Exception as e:
            print(f"Error sending withdrawal request: {e}")
            await event.respond("âš ï¸ Could not send withdrawal request to admin channel. Please contact admin.")
        return

    # --- Staking: Claim farming reward ---
    if data == "claim_farming":
        if not await is_user_joined_all(user_id) and not await is_admin(user_id):
            await event.answer("âŒ You must join all required channels first.", alert=True)
            return
        async with db_pool.acquire() as conn:
            user = await conn.fetchrow("SELECT last_bonus, hold_balance FROM users WHERE user_id=$1", user_id)
            if not user:
                await event.answer("âŒ User not found.", alert=True)
                return
            last_claim = user['last_bonus']
            now = int(time.time())
            if last_claim and now - last_claim < 86400:
                ...
                await event.answer(f"â³ You can claim again in {hours}h {minutes}m.", alert=True)
                return
            settings = await get_settings()
            daily_bonus = settings['daily_bonus']
            if daily_bonus <= 0:
                await event.answer("âŒ Daily bonus is not set by admin.", alert=True)
                return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_cat_"):
        if not is_user_admin:
            await event.answer("âŒ Admin only.", alert=True)
            return
        category = data.split("_")[2]
        if category == "cancel":
            await event.edit("âŒ Edit cancelled.")
            return
        channels = []
        if category == "joining":
            async with db_pool.acquire() as conn:
            ...
            if ch:
                channels = [(0, ch)]
        if not channels:
            await event.edit(f"âŒ No channels in {category} category.")
            return
        rows = []
        for cid, ch in channels:
            rows.append([Button.inline(f"âœï¸ @{ch}", f"edit_sel_{category}_{cid}")])
        rows.append([Button.inline("ðŸ”™ Cancel", b"edit_cat_cancel")])
        await event.edit(f"ðŸ“ Select a channel to edit in **{category}**:", buttons=rows)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_sel_"):
        ...
        await event.edit(f"âœï¸ Please send the new channel username or link for the **{category}** channel:")
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("del_cat_"):
        ...
        category = data.split("_")[2]
        if category == "cancel":
            await event.edit("âŒ Delete cancelled.")
            return
        channels = []
        if category == "joining":
            async with db_pool.acquire() as conn:
        ...
        rows = []
        for cid, ch in channels:
            rows.append([Button.inline(f"ðŸ—‘ï¸ @{ch}", f"del_sel_{category}_{cid}")])
        rows.append([Button.inline("ðŸ”™ Cancel", b"del_cat_cancel")])
        await event.edit(f"ðŸ—‘ï¸ Select a channel to delete from **{category}**:", buttons=rows)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("del_sel_"):
        ...
        if category == "joining":
            ...
            if category == "withdrawal":
                ...
                ch_name = "Unknown"
        buttons = [
            [Button.inline("âœ… Yes, delete", f"confirm_del_ch_{category}_{cid}")],
            [Button.inline("âŒ Cancel", f"del_cat_{category}")]
        ]
        await event.edit(f"âš ï¸ Are you sure you want to delete **@{ch_name}** from **{category}**?", buttons=buttons)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("confirm_del_ch_"):
        ...
        if category == "joining":
            ...
            await event.edit("âœ… Joining channel deleted.")
        elif category == "withdrawal":
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE settings SET withdrawal_channel = '' WHERE id=1")
            await event.edit("âœ… Withdrawal channel cleared.")
        elif category == "proof":
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE settings SET task_proof_channel = '' WHERE id=1")
            await event.edit("âœ… Proof channel cleared.")
        elif category == "task":
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE settings SET task_channel = '' WHERE id=1")
            await event.edit("âœ… Task channel cleared.")
        else:
            await event.edit("âŒ Unknown category.")
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("tg_admin_check_"):
        ...
        if not state or state.get('channel') != channel_username:
            await event.answer("âŒ Invalid session.", alert=True)
            return
        entity = await is_bot_admin_in_channel(channel_username)
        if entity:
            state['step'] = 'limit'
            await event.edit(f"âœ… Bot is admin of @{channel_username}.\n\nðŸš€ **Task Configuration Service**\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”\nðŸŽ¯ Target Channel: https://t.me/{channel_username}\n\nEnter the number of services you want to process.\n\nâš ï¸ Requirement: Minimum 5\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”", 
                             buttons=[[Button.inline("ðŸ”™ Back", f"tg_admin_edit_{channel_username}")]])
            await event.answer("âœ… Admin verified!", alert=True)
        else:
            await event.answer("âŒ Bot is not admin or not found. Add bot as admin and try again.", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("tg_admin_edit_"):
        ...
        if state:
            state['step'] = 'channel'
            await event.edit("ðŸ“¢ Please send the channel username or link again:")
        else:
            await event.answer("âŒ Session expired.", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data == "tg_admin_cancel":
        ...
        if user_id in admin_tg_task_state:
            del admin_tg_task_state[user_id]
        await event.edit("âŒ Task creation cancelled.")
        await event.answer("Cancelled.", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and (data.startswith("tg_admin_confirm_yes") or data.startswith("tg_admin_confirm_no")):
        ...
        state = admin_tg_task_state.get(user_id)
        if not state:
            await event.answer("âŒ No session.", alert=True)
            return
        ...
        await event.edit(f"âœ… **TG Task Added Successfully!**\n\n"
                         f"ðŸ“¢ Channel: https://t.me/{channel}\n"
                         f"ðŸ‘¥ Task Limit: {limit}\n"
                         f"ðŸ’° Reward per task: {reward} {currency}")
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_task_"):
        parts = data.split("_")
        if len(parts) != 4:
            await event.answer("âŒ Invalid data.", alert=True)
            return
        tid = int(parts[2])
        task_type = parts[3]
        if not is_user_admin:
        ...
        keyboard = [
            [Button.inline("ðŸ”— URL", f"edit_field_{tid}_url"),
             Button.inline("â±ï¸ Time", f"edit_field_{tid}_time")],
            [Button.inline("ðŸ’° Reward", f"edit_field_{tid}_reward"),
             Button.inline("ðŸ‘¥ User Limit", f"edit_field_{tid}_limit")],
            [Button.inline("ðŸ“Ž Proof Type", f"edit_field_{tid}_proof"),
             Button.inline("âŒ Cancel", f"edit_cancel_{tid}")]
        ]
        await event.edit("ðŸ“ **Edit Task**\n\nSelect what you want to change:", buttons=keyboard)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_field_"):
        ...
        if user_id not in admin_edit_state or admin_edit_state[user_id]['task_id'] != tid:
            await event.answer("âŒ No editing session.", alert=True)
            return
        ...
        if not col:
            await event.answer("âŒ Invalid field.", alert=True)
            return
        ...
        prompt_map = {
            'url': "ðŸ”— Enter new URL (including http:// or https://):",
            'time': "â±ï¸ Enter new time required in seconds (e.g., 30):",
            'reward': "ðŸ’° Enter new reward amount:",
            'limit': "ðŸ‘¥ Enter new user limit (number):",
            'proof': "ðŸ“Ž Enter new proof type (send `screenshot`, `screen record`, or `skip`):"
        }
        prompt_msg = prompt_map.get(field, "Enter new value:")
        back_btn = Button.inline("ðŸ”™ Back", f"edit_back_{tid}")
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_confirm_"):
        ...
        if new_val is None:
            await event.answer("âŒ No value to update.", alert=True)
            return
        ...
        try:
            ...
            await event.answer(f"âŒ Database error: {e}", alert=True)
            return
        ...
        display = display_names.get(col, col)
        success_text = f"âœ… {display} updated successfully!\n\nNew value: {new_val}"
        buttons = [
            [Button.inline("ðŸ”™ Back", f"edit_back_{tid}"),
             Button.inline("âŒ Cancel", f"edit_cancel_{tid}")]
        ]
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("edit_cancel_"):
        ...
        await event.edit("âŒ Editing cancelled.")
        return
    ...
    if data == "task_cancel_list":
        ...
        if user_id in task_list_msgs:
            ...
            try:
                await client.edit_message(list_info['chat_id'], list_info['msg_id'], "âŒ Cancelled.")
            except:
                pass
            del task_list_msgs[user_id]
        await event.respond("ðŸ”™ Back to Task Menu.", buttons=task_buttons)
        return
    ...
    if data.startswith("start_task_"):
        try:
            ...
            await event.answer("âŒ Invalid task.", alert=True)
            return

        async with db_pool.acquire() as conn:
            task = await conn.fetchrow("SELECT id, task_type, url, time_required, reward, task_limit, completed_count, proof_type, status FROM tasks WHERE id=$1", task_id)
            if not task or task['status'] != 'active':
                await event.answer("âŒ This task is no longer available.", alert=True)
                await client.edit_message(event.chat_id, event.message_id, "âŒ **Task Unavailable**\n\nThe task you selected has been removed or is no longer active.")
                return

            if task['completed_count'] >= task['task_limit']:
                await event.answer("âŒ This task is already full.", alert=True)
                await client.edit_message(event.chat_id, event.message_id, f"âŒ **Task Full**\n\nThe task has already been completed by the maximum number of users ({task['task_limit']}). Please try another task.")
                return

            existing = await conn.fetchval("SELECT status FROM task_submissions WHERE user_id=$1 AND task_id=$2 AND status IN ('pending', 'approved')", user_id, task_id)
            if existing:
                status_text = "pending" if existing == 'pending' else "completed"
                await event.answer(f"âŒ You have already {status_text} this task.", alert=True)
                await client.edit_message(event.chat_id, event.message_id, f"âŒ **Already {status_text.title()}**\n\nYou have already {status_text} this task. You cannot start it again.")
                return
        ...
        await render_task_details(user_id, chat_id, msg_id)
        await event.answer("âœ… Task loaded.", alert=False)
        return
    ...
    if data.startswith("task_visited_"):
        try:
            ...
            await event.answer("âŒ Invalid.", alert=True)
            return

        if user_id not in task_sessions:
            await event.answer("âŒ No active session.", alert=True)
            return

        session = task_sessions[user_id]
        if session['task_id'] != task_id:
            await event.answer("âŒ Task mismatch.", alert=True)
            return

        if session.get('visited', False):
            await event.answer("â³ Timer already started. Please wait.", alert=True)
            return

        session['start_time'] = int(time.time())
        session['visited'] = True

        text = (f"â³ **Timer Started!**\n\n"
                f"Please wait **{session['time_required']} seconds**.\n"
                f"After the timer ends, you will be able to claim reward.\n\n"
                f"â±ï¸ You can visit the website again if you wish.")
        buttons = [
            [Button.inline(f"â³ Please wait {session['time_required']} seconds...", f"task_wait_{task_id}")],
            [Button.inline("ðŸ”™ Back", b"task_back")]
        ]
    ...
    if data.startswith("task_wait_"):
        ...
        if elapsed < required:
            ...
            await event.answer(f"â³ Please wait {remaining} more seconds.", alert=True)
        else:
            await event.answer("â³ Please wait for the timer to finish.", alert=True)
        return
    ...
    if data.startswith("claim_task_"):
        ...
        if session.get('proof_type') == 'skip':
            await event.answer("âœ… This task does not require proof.", alert=True)
            return
        ...
        proof_type = session.get('proof_type', 'screenshot')
        proof_instruction = "ðŸ“¸ Please take a screenshot and send the photo here." if proof_type == 'screenshot' else "ðŸŽ¥ Please record your screen and send the video here."

        text = (f"ðŸ“Ž **Proof Required!**\n\n"
                f"{proof_instruction}\n\n"
                f"âš ï¸ Your proof will be reviewed by admin.")
        buttons = [
            [Button.inline("ðŸ”™ Back", b"task_back")]
        ]
        await client.edit_message(session['chat_id'], session['message_id'], text, buttons=buttons)
    ...
    if data.startswith("tg_check_"):
        ...
        try:
            ...
            await event.answer("âŒ Could not resolve channel.", alert=True)
            return

        if is_member:
            text = (f"âœ… **You are a member of the channel!**\n\n"
                    f"Click the **Claim Reward** button to get your reward.")
            buttons = [
                [Button.inline("ðŸŽ Claim Reward", f"claim_tg_{task_id}")],
                [Button.inline("ðŸ”™ Back", b"task_back")]
            ]
            await client.edit_message(session['chat_id'], session['message_id'], text, buttons=buttons)
            session['screen'] = 'claim_ready'
            session['prev_screen'] = 'details'
            await event.answer("âœ… Membership verified!", alert=True)
        else:
            await event.answer("âŒ You are not a member. Please join the channel first.", alert=True)
        return
    ...
    if data.startswith("claim_tg_"):
        ...
        async with db_pool.acquire() as conn:
            if await conn.fetchval("SELECT status FROM task_submissions WHERE user_id=$1 AND task_id=$2 AND status='approved'", user_id, task_id):
                await event.answer("âŒ You have already claimed this task.", alert=True)
                return
        ...
        await event.answer("âœ… Reward claimed!", alert=True)
        return
    ...
    if data.startswith("approve_sub_") or data.startswith("reject_sub_"):
        ...
        proof_channel_str = settings['task_proof_channel']
        if not proof_channel_str:
            await event.answer("âŒ Proof channel not set.", alert=True)
            return
        try:
            proof_channel = await client.get_entity(proof_channel_str)
        except:
            await event.answer("âŒ Proof channel not found.", alert=True)
            return

        is_channel_admin = await is_user_admin_in_channel(user_id, proof_channel)
        if not is_channel_admin:
            await event.answer("âŒ You are not an admin of the proof channel.", alert=True)
            return
        ...
        async with db_pool.acquire() as conn:
            ...
            if not sub:
                await event.answer("âŒ Submission not found or already processed.", alert=True)
                return
            ...
            if data.startswith("approve_sub_"):
                ...
                msg_obj = await event.get_message()
                new_text = msg_obj.text + f"\n\nðŸ”´ **Status: Rejected by Admin**"
                await event.edit(new_text, buttons=None)
                try:
                    await client.send_message(sub['user_id'], f"âŒ **Task Rejected**\n\nYour task submission has been rejected. Please ensure you visited the website correctly and submitted valid proof.")
                except:
                    pass
                await event.answer("âŒ Rejected.", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("del_task_"):
        ...
        if not is_user_admin:
            ...
            return
        kb = [
            [Button.inline("âœ… Yes, delete", f"confirm_del_task_{task_id}")],
            [Button.inline("âŒ Cancel", b"cancel_del_task")]
        ]
        await event.edit(f"âš ï¸ Are you sure you want to delete this {task_type} task?", buttons=kb)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("confirm_del_task_"):
        try:
            ...
            await event.answer("âŒ Invalid task ID.", alert=True)
            return
        if not is_user_admin:
            await event.answer("âŒ Admin only.", alert=True)
            return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and (data.startswith("confirm_task_yes") or data.startswith("confirm_task_no")):
        ...
        await event.edit(f"âœ… **Media Task Added Successfully!**\n\n"
                         f"ðŸ”— URL: {task_data['url']}\n"
                         f"â±ï¸ Time: {task_data['time_required']} seconds\n"
                         f"ðŸ’° Reward: {task_data['reward']} {currency}\n"
                         f"ðŸ‘¥ User Limit: {task_data['task_limit']}\n"
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and (data.startswith("confirm_add_balance_yes") or data.startswith("confirm_add_balance_no")):
        ...
        del admin_confirm[user_id]
        await event.edit(f"âœ… Added {amount:.2f} {currency} to user {target_user} (Real Balance).")
        try:
            await client.send_message(target_user, f"ðŸŽ **Admin added {amount:.2f} {currency} to your Real Balance!**")
        except:
            pass
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and (data.startswith("confirm_cut_balance_yes") or data.startswith("confirm_cut_balance_no")):
        ...
        async with db_pool.acquire() as conn:
            ...
            if balance < amount:
                await event.edit(f"âŒ Insufficient balance. User has {balance:.2f} {currency}.")
                del admin_confirm[user_id]
                return
            await conn.execute("UPDATE users SET balance = balance - $1 WHERE user_id=$2", amount, target_user)
        del admin_confirm[user_id]
        await event.edit(f"âœ… Cut {amount:.2f} {currency} from user {target_user} (Real Balance).")
        try:
            await client.send_message(target_user, f"âš ï¸ **Admin deducted {amount:.2f} {currency} from your Real Balance.**")
        except:
            pass
        await event.answer("Done!", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and (data.startswith("confirm_set_bonus_yes") or data.startswith("confirm_set_bonus_no")):
        ...
        del admin_confirm[user_id]
        await event.edit(f"âœ… Bonus set: {bonus_amount:.2f} {currency} for {ref_count} referrals.\n\nUsers will get this bonus for every multiple of {ref_count} referrals.")
        await event.answer("Done!", alert=True)
        return
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("delch_"):
        ...
        async with db_pool.acquire() as conn:
            ...
            if not ch:
                ...
                return
        kb = [
            [Button.inline("âœ… Yes, delete", f"confirm_del_{ch_id}")],
            [Button.inline("âŒ Cancel", b"cancel_del")]
        ]
    ...
    if is_user_admin and not admin_user_mode.get(user_id, True) and data.startswith("confirm_del_"):
        ...
        await event.edit("âœ… Channel removed from join list.")
        async with db_pool.acquire() as conn:
        ...
        if remaining:
            ...
            if row:
                rows.append(row)
            rows.append([Button.inline("ðŸ”™ Done", b"cancel_del")])
    ...
    if data.startswith("p_"):
        ...
        msg_obj = await event.get_message()
        new_text = msg_obj.text + f"\n\nAdd transactions link\nðŸ›¡ Status: 100% Verified & Paid"
        await event.edit(new_text, buttons=None)

        success_text = (
            f"ðŸŽ **Congratulations!**\n\n"
            f"âœ… **Your withdrawal of {net:.2f} {currency} (after fee) has been approved and sent!**\n\n"
            f"ðŸ•’ **Your Withdraw Date & Time was:** {request_time_str}\n\n"
            f"ðŸ’¼ Check your wallet now."
        )
        try:
            await client.send_message(target_user, success_text)
            await event.answer("âœ… Success! User notified.", alert=True)
        except:
            await event.answer("âš ï¸ Could not notify user.", alert=True)
        return
    ...
    if data == "my_ref":
        ...
        ref_list = "\n".join([f"ðŸ‘¤ @{r['username']}" if r['username'] != "No Username" else f"ðŸ‘¤ {r['user_id']}" for r in refs]) or "No referrals yet."
        msg = f"âž¡ï¸ Your Total Referrals: {len(refs)}\n\nðŸ‘¨â€ðŸ‘¨â€ðŸ‘¦ Your Referred Users â¬‡ï¸\n\n{ref_list}"
        await event.edit(msg, buttons=[Button.inline("ðŸ”™ Back", b"back_inv")])
        return
    ...
    if data == "top_list":
        ...
        if not tops:
            await event.edit("âŒ No users found.", buttons=[Button.inline("ðŸ”™ Back", b"back_inv")])
            return

        header = f"ðŸ† Top 25 Referral Leaders\n\nðŸ“Š Ranking by Referrals\n\n"
        lines = []
        for i, row in enumerate(tops, start=1):
            ...
            lines.append(f"{rank_str} {display} â€” {total_ref} refs Â· {total_earned:.2f} {currency}")

        top_msg = header + "\n".join(lines) + "\n\nðŸ“Š Rankings update in real-time. Keep inviting to climb the leaderboard! ðŸš€"
        await event.edit(top_msg, buttons=[Button.inline("ðŸ”™ Back", b"back_inv")])
        return
